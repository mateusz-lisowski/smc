...
SERIAL_PORT = '/dev/serial/by-id/usb-FTDI_FT231X_USB_UART_D30EZEGB-if00-port0'
...
serial_list = mavutil.auto_detect_serial(
    preferred_list=['*CP210X*', '*FTDI*', "*Arduino_Mega_2560*", "*3D_Robotics*", "*USB_to_UART*", '*PX4*', '*FMU*', "*Gumstix*"]
)
...
if __name__ == "__main__":
    ...
    while not is_influxdb_connected and retry_count < max_retries:
        try:
            print(f"Attempting to connect (Retry {retry_count + 1} of {max_retries})...")
    ...
    if not is_influxdb_connected:
        print(f"Failed to establish InfluxDB connection after {max_retries} retries.")
    ...
    while not is_mavlink_connected and retry_count < max_retries:
        try:
            ...
            mavcon = mavutil.mavlink_connection(SERIAL_PORT, baud=baudrate, autoreconnect=True, dialect="rocket")
    ...
    if not is_mavlink_connected:
        print(f"Failed to establish MAVLink connection after {max_retries} retries.")
        exit()

    write_api = client.write_api(write_options=SYNCHRONOUS)
    
    # TODO: 2 threads: one for receiving second for writing data
    # now there is about 3 second delay
    t1 = time.time()
    while True:
        ...
        while True:
            ...
            if title == "SIMBA_ACTUATOR_STATUS":
                ...
                pass
            elif title == "SIMBA_PRESSURE_STATUS":
                pass
            ...
            for key, value in msg_iter:
                try:
                    write_api.write(BUCKET, ORG, Point(title).field(key, value).time(time.time_ns()))
                except Exception as e:
                    pass
            ...
            if t2 - t1 > 1.0:
                try:
                    ...
                    print(e)
                    write_api.write(BUCKET, ORG, Point("BANDWITH").field("bytes_recv", 0.0).time(time.time_ns()))
                    write_api.write(BUCKET, ORG, Point("BANDWITH").field("bytes_sent", 0.0).time(time.time_ns()))
        ...
        while True:
            ...
            for port, desc, hwid in sorted(ports):
                    print("{}: {} [{}]".format(port, desc, hwid))
            try:
                mavcon = mavutil.mavlink_connection(SERIAL_PORT, baud=baudrate, autoreconnect=True)
                print("Waiting for heartbeat...")
                mavcon.mav.heartbeat_send(mavutil.mavlink.MAV_TYPE_ONBOARD_CONTROLLER,
                                                mavutil.mavlink.MAV_AUTOPILOT_INVALID, 0, 0, 0)
                try:
